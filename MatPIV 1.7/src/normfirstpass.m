function [xx,yy,datax,datay]=normfirstpass(A,B,N,ol,idx,idy,maske)

% function [x,y,datax,datay]=firstpass(A,B,M,N,ol,idx,idy,maske)
%
% This function is used in conjunction with one of the MULTIPASS.M
% run-files in MatPIV. 
% Inputs are allocated from within the MULTIPASS* file.
%
% Function uses the normxcorr2_general.m file by Dirk Padfield
% http://www.mathworks.com/matlabcentral/fileexchange/29005-generalized-normalized-cross-correlation
% However, users may need to modify his file to remove dependency on IP
% toolbox 

% 1999 - 2011, J. Kristian Sveen (jks@math.uio.no)
% For use with MatPIV 1.7, Copyright
% Distributed under the terms of the GNU - GPL license
% timestamp: 08:57, 4 March 2011
...
yy=xx; datax=xx; datay=xx;% SnR=xx; Pkh=xx;
    ...
    for ii=1:((1-ol)*M):sx-M+1
        ...
        if IN(jj+N/2,ii+M/2)~=1
            ...
            if isnan(std(C(:)))~=1 && isnan(std(D(:)))~=1 && sum(std(D(:)))~=0 && sum(std(C(:)))~=0
                ...
                R=normxcorr2_general(C,D);%/(N*M*stad1*stad2);
