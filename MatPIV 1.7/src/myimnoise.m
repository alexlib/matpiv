function b = myimnoise(varargin)
%MYIMNOISE Add noise to image.
%   J = MYIMNOISE(I,TYPE,...) Add noise of a given TYPE to the intensity
%   image I. TYPE is a string that can have one of these values:
%
...
%   MYIMNOISE. All numerical parameters are normalized; they correspond
%   to operations with images with intensities ranging from 0 to 1.
%   
%   J = myimnoise(I,'gaussian',M,V) adds Gaussian white noise of mean M
%   and variance V to the image I. When unspecified, M and V default to
%   0 and 0.01 respectively.
%   
%   J = myimnoise(I,'localvar',V) adds zero-mean, Gaussian white noise of
%   local variance, V, to the image I.  V is an array of the same size as I.
%
%   J = myimnoise(I,'localvar',IMAGE_INTENSITY,VAR) adds zero-mean,
%   Gaussian noise to an image, I, where the local variance of the noise
%   is a function of the image intensity values in I. IMAGE_INTENSITY
%   and VAR are vectors of the same size, and PLOT(IMAGE_INTENSITY,VAR)
%   plots the functional relationship between noise variance and image
%   intensity. IMAGE_INTENSITY must contain normalized intensity values
%   ranging from 0 to 1.
%
%   J = myimnoise(I,'poisson') generates Poisson noise from the data
%   instead of adding artificial noise to the data. In order to respect
%   Poisson statistics, the intensities of uint8 and uint16 images must
%   correspond to the number of photons (or any other quanta of
%   information). Double-precision images are used when the number of
%   photons per pixel can be much larger than 65535 (but less than
%   10^12); the intensities values vary between 0 and 1 and correspond
%   to the number of photons divided by 10^12.
%
%   J = myimnoise(I,'salt & pepper',D) adds "salt and pepper" noise to
%   the image I, where D is the noise density. This affects
%   approximately D*numel(I) pixels. The default for D is 0.05.
%   
%   J = myimnoise(I,'speckle',V) adds multiplicative noise to the image
%   I, using the equation J = I + n*I, where n is uniformly distributed
