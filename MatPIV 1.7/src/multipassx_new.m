...
if ~isempty(datax)
  ...
  Wx=floor(sx/dx)/(-overlap+1);  
  Wy=floor(sy/dy)/(-overlap+1);
...
for i=1:iter-1
    ...
    datax=round(interp2(x(1,:),y(:,1),datax,XI,YI'));
    datay=round(interp2(x(1,:),y(:,1),datay,XI,YI'));
...
disp('* Final Pass')
[x,y,u,v,SnR,Pkh,brc] = finalpass_new(A,B,wins(end,:),overlap,round(datax),...
