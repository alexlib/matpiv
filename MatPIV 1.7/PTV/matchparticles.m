...
for i=1:length(xi)
  for j=1:length(xj)
    %if i>length(ui) % | ii==1 % THIS SHOULD BE INCLUDED WHEN NO PIV
    %IS USED
    %  Bij(j,i)=(sqrt((xi(i)-xj(j)).^2 + (yi(i)-yj(j)).^2 )).^2;
    %  maxerror=5; % increase max error to allow longer displacements
    %else
      Bij(j,i) = (sqrt( (xi(i) + ui(i)*dt - xj(j)).^2 +...
			(yi(i) + vi(i)*dt - yj(j)).^2 ) ).^m; 
    %  maxerror=2;
    %end
  end
  if ~any(isnan(Bij(:,i)))
